...
#define GameVer "1.30.0.0"
#define GameMajorVer "30"
#define GameMiddleVer "0"
#define GameMinorVer "0"

#define MyAppName "«WэК» - Возвращение Белого Тигра"
...
OutputBaseFilename=wek_tiger_lesta_{#AppBuild}
...
WizardStyle=classic
WizardImageFile=tiger.bmp
WizardSmallImageFile=portal_small.bmp
...
Source: "{app}\mods\{code:GetVersion}\wek_hangars\*"; DestDir: "{app}\wek_tiger_uninst\backup\{code:GetVersion}\wek_hangars"; Flags: external skipifsourcedoesntexist uninsneveruninstall
...
begin
  ...
  begin
    ...
    begin
      VersionStatusLabel.Caption := Format('Мод был создан для версии %s, и его работоспособность не'#13#10'гарантируется.', [CurVersion]);
      VersionStatusLabel.Font.Color := clRed;
    end
    else if MiddleVersion <> CurMiddleVersion then
    begin
      VersionStatusLabel.Caption := Format('Мод был создан для версии %s, поэтому есть вероятность'#13#10'возникновения проблем совместимости.', [CurVersion]);
      VersionStatusLabel.Font.Color := $0060FF;
    end
    else if MinorVersion <> CurMinorVersion then
    begin
      VersionStatusLabel.Caption := Format('Мод были создан для версии %s, но шанс возникновения проблем'#13#10'минимален.', [CurVersion]);
      VersionStatusLabel.Font.Color := clGreen;
    end
    else
    begin
      VersionStatusLabel.Caption := 'Мод совместим с данной версией игры.';
      VersionStatusLabel.Font.Color := clGreen;
    end;
  end
  else
...
//   WinHttpReq.Open('GET', 'https://download.wot-classic.ru/manifest.json', false);
//   WinHttpReq.Send();
...
//         IDYES:
//           begin
//             ShellExec('open', 'https://download.wot-classic.ru/wek_install.exe', '', '', SW_SHOWNORMAL, ewNoWait, ErrorCode);
//             Result := False;
//           end;
//       end;
//     end;
